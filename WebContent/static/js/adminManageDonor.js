...
$(document).on('click', "#editDonation", function(){
	  ...
	  $('.row-select input:checked').each(function() {
		  ...
		  donationDate = (new Date(donationDate).toISOString('en-US', {
			    year: 'numeric',
			  month: '2-digit',
			day: '2-digit'
			})).split('T')[0];
	      ...
	      success: function (data, textStatus, jqXHR) {	    
	          ...
	          $(JsonData.rows).each(function (index, item) {  
	        	  $('#adminDonationTable').append(
	        			...
	        			'<td style="text-align: center;"><button class="btn btn-primary" type="button" onclick=downloadDonation('+item.pkDonnerId+')  style="font-size: medium;border-radius: 20px;">View</button></td>'+
	        		      '</tr>'
	        	  );
	        	  i++;
		      });
...
*/
		  ...
		  $("#adminDonationTable").html('<tr class="no-records"><td colspan="11" style="text-align:center;text-color:black">Sorry, Something went wrong,Please try again.</td></tr>');
		  $('#myModal').hide();
	    	//Calling Loader
	    	$(".loader1").hide();
