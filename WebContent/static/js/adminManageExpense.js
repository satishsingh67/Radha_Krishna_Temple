...
$(document).on('click', "#editExpenses", function(){
	  ...
	  $('.row-select input:checked').each(function() {
		  ...
		  expenseDate = (new Date(expenseDate).toISOString('en-US', {
			    year: 'numeric',
			  month: '2-digit',
			day: '2-digit'
			})).split('T')[0];
	      ...
	      success: function (data, textStatus, jqXHR) {	    
	          ...
	          $(JsonData.rows).each(function (index, item) {  
	        	  $('#adminExpenseTable').append(
	        			 ...
	        			 '<td style="text-align: center;font-size: 14px;display:none">'+item.pkExpenseId+'</td>'+
	        			...
	        			'<td style="text-align: center;"><button class="btn btn-primary" type="button" onclick=downloadExpense('+item.pkExpenseId+')  style="font-size: medium;border-radius: 20px;">View</button></td>'+
	        		      '</tr>'
	        	  );
	        	  i++;
		      });
